...
### Hotkey Access
- Press `⌥ + Space` (Option + Space) from any screen or application to open the Quick Capture floating dialog.
- Press `Esc` to close Quick Capture.
...
3. **Platform Mocking in Tests**:
   - In unit and widget tests, always provide mock platform services (`MockHotKeyService`, `MockTrayService`) to prevent desktop plugin crashes in headless environments.
...
```text
lib/
├── core/                  # Database manager, theme, exceptions, platform bridges
...
│   └── services/          # Pure business logic services (TimerService, IdleService)
├── data/                  # SQLite schema, migrations, concrete repositories
│   ├── database/          # Tables definition
│   ├── migrations/        # Versioned migrations (MigrationV1)
...
└── features/              # Feature UI, Notifiers, and dialogs
    ...
    ├── quick_capture/     # Floating keyboard-first capture UI
