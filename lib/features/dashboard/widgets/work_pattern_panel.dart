...
/// The dashboard's "what keeps happening" section.
///
/// The breakdown cards above it answer where a period's hours went. This
...
///
/// It reads its own trailing window rather than the dashboard's Today / Week /
/// Month selector — a pattern needs history to be a pattern.
class WorkPatternPanel extends ConsumerWidget {
  const WorkPatternPanel({super.key});
...
class _PanelHeader extends StatelessWidget {
  final PatternWindow window;
  final WorkPatternReport? report;
  final ValueChanged<PatternWindow> onWindowChanged;
  ...
  Widget build(BuildContext context) {
    ...
    final subtitle = switch (report) {
      null => 'Scanning ${window.label.toLowerCase()}…',
      final r when !r.hasData =>
        'Nothing tracked in the ${window.days} days scanned',
      final r => '${r.sessionCount} sessions over '
          '${r.rhythm.trackedDayCount} tracked '
          '${r.rhythm.trackedDayCount == 1 ? 'day' : 'days'} · '
          '${TimerService.formatDuration(r.totalActive, compact: true, includeSeconds: false)} active',
    };
        ...
        Row(
          ...
          children: [
            Icon(Icons.insights_outlined,
                size: IconSizes.xl, color: colors.accent),
            const SizedBox(width: Spacing.sm + 2),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              mainAxisSize: MainAxisSize.min,
              children: [
                Text('Patterns & Signals', style: theme.textTheme.titleLarge),
          ...
          children: [
            AppSegmentedControl<PatternWindow>(
              ...
              options: const [
                ...
                SegmentOption(
                    value: PatternWindow.oneQuarter, label: '90 days'),
              ],
            ),
            if (report != null && report!.hasInsights) ...[
              ...
              Tooltip(
                ...
                child: IconButton(
                  ...
                  style: IconButton.styleFrom(
                    minimumSize:
                        const Size(ControlSizes.toolbar, ControlSizes.toolbar),
                    maximumSize:
                        const Size(ControlSizes.toolbar, ControlSizes.toolbar),
  ...
  /// The findings, ready to paste into a 1:1 agenda or a handover note.
  ///
  /// Delegating something means telling somebody about it, and retyping four
  /// paragraphs of figures is exactly the friction that stops the insight ever
  /// leaving the dashboard.
  void _copy(BuildContext context, WorkPatternReport report) {
  ...
  Widget build(BuildContext context) {
    ...
    final stats = <_RhythmStat>[
      _RhythmStat(
        ...
        detail: TimerService.formatDuration(rhythm.deepWorkTotal,
      ...
      _RhythmStat(
        ...
        value: TimerService.formatDuration(rhythm.longestUnbrokenBlock,
            compact: true, includeSeconds: false),
        detail: 'unbroken',
...
class _InsightLanes extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // An empty lane is dropped rather than rendered as a heading with nothing
    // under it, so a window with only one kind of finding gets the full width.
    final lanes = <_Lane>[
      for (final action in InsightAction.values)
...
/// One column of the panel: everything the reader can do the same thing about.
class _Lane {
...
class _InsightCardState extends State<_InsightCard> {
  ...
  Widget build(BuildContext context) {
    ...
    // The subject's own colour where it has one, so a card about a project
    // reads as that project; the lane's colour otherwise.
          ...
          Row(
            ...
            children: [
              ...
              if (insight.timeInvolved > Duration.zero) ...[
                const SizedBox(width: Spacing.sm),
                StatusBadge(
                  label: TimerService.formatDuration(insight.timeInvolved,
                      compact: true, includeSeconds: false),
                  icon: Icons.schedule,
