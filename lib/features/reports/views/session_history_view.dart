  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref, String sessionId, String taskName) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Delete Session', style: TextStyle(color: AppTheme.textPrimaryDark)),
        content: Text('Are you sure you want to delete this session for "$taskName"? This action cannot be undone.', style: const TextStyle(color: AppTheme.textSecondaryDark)),
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: const Text('Cancel', style: TextStyle(color: AppTheme.textSecondaryDark))),
          ElevatedButton(
          ...
          children: [
            ...
            Expanded(
              child: sessionsAsync.when(
                ...
                error: (err, stack) => Center(child: Text('Error loading sessions: $err', style: const TextStyle(color: AppTheme.accentRed))),
                data: (records) {
                  if (records.isEmpty) {
                    return Center(
                      child: Column(
                        ...
                        children: [
                          Icon(Icons.history_toggle_off, size: 48, color: AppTheme.textSecondaryDark.withValues(alpha: 0.5)),
                          const SizedBox(height: 12),
                          const Text(
                            'No sessions recorded in this period',
                            style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: AppTheme.textSecondaryDark),
                          ),
                        ],
                      ),
                    );
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(dateStr, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppTheme.textPrimaryDark)),
                                  const SizedBox(height: 2),
                                  Text('$startStr – $endStr', style: const TextStyle(fontSize: 11, color: AppTheme.textSecondaryDark)),
                                ],
                              ),
                            ),
                            const SizedBox(width: 12),
                            ...
                            if (record.category != null) ...[
                              Icon(IconUtils.getIcon(record.category!.iconName), size: 14, color: AppTheme.textSecondaryDark),
                              const SizedBox(width: 4),
                              Text(record.category!.name, style: const TextStyle(fontSize: 11, color: AppTheme.textSecondaryDark)),
                              const SizedBox(width: 12),
                            ],
                            ...
                            // Edit Button
                            IconButton(
                              icon: const Icon(Icons.edit_outlined, size: 16, color: AppTheme.textSecondaryDark),
