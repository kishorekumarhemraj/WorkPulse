...
class SessionHistoryView extends ConsumerWidget {
  ...
  Future<void> _pickCustomRange(BuildContext context, WidgetRef ref) async {
    ...
    if (picked != null) {
      ...
      ref.read(reportsTimeRangeProvider.notifier).setRange(DashboardTimeRange.custom);
    }
  }

  Future<void> _confirmDelete(BuildContext context, WidgetRef ref, String sessionId, String taskName) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: AppTheme.getColors(context).surface,
        title: Text('Delete Session', style: TextStyle(color: AppTheme.getColors(context).textPrimary)),
        content: Text('Are you sure you want to delete this session for "$taskName"? This action cannot be undone.', style: TextStyle(color: AppTheme.getColors(context).textSecondary)),
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: Text('Cancel', style: TextStyle(color: AppTheme.getColors(context).textSecondary))),
          ElevatedButton(
          ...
          children: [
            ...
            Row(
              children: [
                      ...
                      return InkWell(
                        onTap: () {
                          if (r == DashboardTimeRange.custom) {
                            ...
                            ref.read(reportsTimeRangeProvider.notifier).setRange(r);
                          }
                        },
                ...
                IconButton(
                  ...
                  icon: Icon(Icons.refresh, size: 18, color: AppTheme.getColors(context).textSecondary),
            ...
            Expanded(
              child: sessionsAsync.when(
                ...
                error: (err, stack) => Center(child: Text('Error loading sessions: $err', style: TextStyle(color: AppTheme.accentRed))),
                data: (records) {
                  if (records.isEmpty) {
                    return Center(
                      child: Column(
                        ...
                        children: [
                          Icon(Icons.history_toggle_off, size: 48, color: AppTheme.getColors(context).textSecondary.withValues(alpha: 0.5)),
                          SizedBox(height: 12),
                          Text(
                            'No sessions recorded in this period',
                            style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: AppTheme.getColors(context).textSecondary),
                          ),
                        ],
                      ),
                    );
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final endStr = s.endTime != null ? timeFormat.format(s.endTime!.toLocal()) : 'Running';
                      final dateStr = dateFormat.format(s.startTime.toLocal());
                      final netDurationStr = TimerService.formatDuration(record.netActiveDuration, includeSeconds: false);
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(dateStr, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppTheme.getColors(context).textPrimary)),
                                  SizedBox(height: 2),
                                  Text('$startStr – $endStr', style: TextStyle(fontSize: 11, color: AppTheme.getColors(context).textSecondary)),
                                ],
                              ),
                            ),
                            SizedBox(width: 12),
                              ...
                              Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(width: 5),
                                    Text(record.project!.name, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: projectColor)),
                                  ],
                                ),
                              ),
                              SizedBox(width: 8),
                            ...
                            if (record.category != null) ...[
                              Icon(IconUtils.getIcon(record.category!.iconName), size: 14, color: AppTheme.getColors(context).textSecondary),
                              SizedBox(width: 4),
                              Text(record.category!.name, style: TextStyle(fontSize: 11, color: AppTheme.getColors(context).textSecondary)),
                              SizedBox(width: 12),
                            ],
                                ...
                                children: [
                                  ...
                                  if (record.workItem.notes != null && record.workItem.notes!.isNotEmpty) ...[
                                    ...
                                    Text(
                                      ...
                                      style: TextStyle(fontSize: 11, fontStyle: FontStyle.italic, color: AppTheme.getColors(context).textSecondary),
                                      maxLines: 1,
                                      overflow: TextOverflow.ellipsis,
                                    ),
                                  ],
                            ...
                            Container(
                              ...
                              child: Text(
                                netDurationStr,
                                style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: AppTheme.accentGreen),
                              ),
                            ),
                            SizedBox(width: 12),
                            ...
                            // Edit Button
                            IconButton(
                              icon: Icon(Icons.edit_outlined, size: 16, color: AppTheme.getColors(context).textSecondary),
                              tooltip: 'Edit session',
                              onPressed: () => SessionEditDialog.show(context, record),
                            ),
                            ...
                            IconButton(
                              ...
                              onPressed: () => _confirmDelete(context, ref, s.id, record.workItem.name),
                            ),
                          ],
                        ),
                      );
